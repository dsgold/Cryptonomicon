  ...
  <div class="container mx-auto flex flex-col items-center p-4">
    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input
                  v-model="ticker"
                  @keydown.enter="checkUniq"
                  @input="isUniq = isExist = false;"
                  type="text"
                  name="wallet"
                  id="wallet"
            ...
            >
              <span
                  v-for="coin in quickPick"
                  :key="coin"
                  @click="
                  ticker = coin;
                  checkUniq();
                "
                  @keydown="isUniq = true; isExist=false;"
                  class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-pink-300 text-gray-800 cursor-pointer"
              >
                {{ coin }}
              </span>
            </div>
            <div v-if="isUniq" class="text-sm text-purple-800">
              Такой тикер уже добавлен
            </div>
            <div v-if="isExist" class="text-sm text-red-800">
              Такой валюты нет
            </div>
          </div>
        </div>
      ...
      <section v-if="selectedTicker" class="relative">
        <h3 class="text-lg leading-6 font-medium text-gray-300 my-8">
          {{ selectedTicker.name }} - USD
        </h3>
        <div class="flex items-end border-black-600 border-b border-l h-64">
          <div
...
import {getCoinList, unsubscribeFromTicker, subscribeToTicker} from "@/api";
...
export default {
  ...
  watch: {
    ticker() {
      this.ticker.length ?
          this.quickPick = Object.values(this.coinList)
              .filter(el => el.indexOf(this.ticker.toUpperCase()) > -1)
              .sort()
              .slice(0, 4)
              .reverse()
          : this.quickPick = ["BTC", "DOGE", "BCH", "CHD"]
    },
    selectedTicker() {
      this.graph = [];
    },
